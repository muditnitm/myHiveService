@php
    $old_path = "packages/workdo/{$slug}/src/marketplace";
@endphp
        ...
        <div class="col-md-12 col-12">
            <div class="form-group">
                ...
                <div class="logo-content mt-4 pb-5">
                    <img id="image" src="{{ check_file($dedicated_theme['dedicated_theme_section_image']) ? get_file($dedicated_theme['dedicated_theme_section_image']) : asset("{$old_path}/image{$image_no}.png") }}"
                        class="w-20 h-20 logo"  style="filter: drop-shadow(2px 3px 7px #011C4B);">
                </div>
        ...
        <div class="border rounded p-3" >
            ...
            @if (isset($dedicated_theme['dedicated_theme_section_cards']) )
                @foreach (( $dedicated_theme['dedicated_theme_section_cards']) as $key => $card)
                    <div id="{{ 'add-cards'.$key }}" class="row">
                        <div class="col-md-10 col-12">
                            <div class="form-group">
                                {{ Form::label('Title', __('Title'), ['class' => 'form-label']) }}
                                {{ Form::text('dedicated_theme_section_cards['.$key.'][title]',$card['title'], ['class' => 'form-control', 'placeholder' => __('Enter Title'),'required' => 'required']) }}
                            </div>
                            <div class="form-group">
                                {{ Form::label('Description', __('Description'), ['class' => 'form-label']) }}
                                {{ Form::text('dedicated_theme_section_cards['.$key.'][description]',$card['description'], ['class' => 'form-control ', 'placeholder' => __('Enter Description'),'required' => 'required']) }}
                            </div>
                        </div>
                        <div class="col-md-2 col-12 d-flex text-center align-items-center justify-content-end popup-action-btn ">
