...
@endphp
{{Form::model(null, array('route' => array('marketplace_screenshots_update',[$slug, $key]), 'method' => 'POST','enctype' => "multipart/form-data",'class'=>'needs-validation','novalidate')) }}
            ...
            <div class="form-group mb-0">
                {{ Form::label('screenshot', __('Screenshot'), ['class' => 'form-label']) }}
                    <div class="logo-content my-4 ">
                        <img id="image" class="w-20 h-20 logo" src="{{ check_file($screenshot['screenshots']) ? get_file($screenshot['screenshots']) : asset("{$old_path}/image{$image_no}.png") }}"
