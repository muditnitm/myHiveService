...
class MarketPlaceController extends Controller
{
    ...
    public function dedicatedindex($slug = null)
    {
        ...
        if (\Auth::user()->isAbleTo('landingpage manage')) {
            ...
            $modules = getshowModuleList();
            return view('landing-page::marketplace.dedicated.index', compact('settings', 'slug', 'dedicated_theme_sections','modules'));
        } else {
    ...
    public function dedicated_theme_edit($slug, $key, $image_no)
    {
        $settings = MarketplacePageSetting::settings($slug);
        $module = getshowModuleList();
        $dedicated_themes = json_decode($settings['dedicated_theme_sections'], true);
        $dedicated_theme = $dedicated_themes[$key];
        return view('landing-page::marketplace.dedicated.edit', compact('dedicated_theme', 'key', 'slug', 'image_no', 'module'));
    }
    ...
    public function whychooseindex($slug = null)
    {
        ...
        if (Auth::user()->isAbleTo('landingpage manage')) {
            ...
            $settings = MarketplacePageSetting::settings($slug);
            $modules = getshowModuleList();
            return view('landing-page::marketplace.whychoose.index', compact('settings', 'slug','modules'));
        } else {
    ...
    public function screenshotindex($slug = null)
    {
        if (\Auth::user()->isAbleTo('landingpage manage')) {
            ...
            $modules = getshowModuleList();
            return view('landing-page::marketplace.screenshots.index', compact('screenshots', 'settings', 'slug','modules'));
        } else {
    ...
    public function screenshots_store(Request $request, $slug)
    {
        ...
        return redirect()->back()->with(['success' => 'The Screenshot has been created successfully']);
    }

    public function screenshots_edit($slug, $key, $image_no)
    {
        $module = getshowModuleList();
        $settings = MarketplacePageSetting::settings($slug);
        $screenshots = json_decode($settings['screenshots'], true);
        ...
        return view('landing-page::marketplace.screenshots.edit', compact('screenshot', 'key', 'slug', 'image_no', 'module'));
    }
    ...
    public function addonindex($slug = null)
    {
        ...
        if (Auth::user()->isAbleTo('landingpage manage')) {
            $modules = getshowModuleList();
            $settings = MarketplacePageSetting::settings($slug);
            return view('landing-page::marketplace.addon.index', compact('settings', 'slug','modules'));
        } else {
